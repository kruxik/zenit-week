...
## S5 — Assets ✅
- [x] T5.1 — `scripts/og-image.mjs`: dead `.gear-btn, .week-nav-btn` selectors dropped,
      the multi-line-label handling simplified to the root's single `<text>`, `#week-bar`
      added to the overlay strip list, header comment corrected.
- [x] T5.2 — `npm run og` → `og-image{,-cs}.{png,svg}` regenerated and eyeballed.
- [x] T5.3 — `npm run screenshots` + `npm run hero:svg` from the canonical week; 6 PNGs,
      6 SVG mockups and `hero.svg` regenerated.
- [x] T5.4 — `npm run build` green; `csp-hashes` wrote the same 8 hashes, confirming no
      inline script was added. **The build also rewrites `APP_VERSION` in place** — a
      deploy-time substitution, reverted rather than committed.
- [x] T5.5 — No manual CHANGELOG edit: it is generated by git-cliff from conventional
      commits (`cliff.toml`). Verified with `npx git-cliff --unreleased` — all three
      `feat` commits land under **Added** and the `refactor` under **Changed**. A
      hand-written entry would have been duplicated at release.

### S5 decisions
- **The marketing root keeps its relabel, and the ring stays.** Signed out — which a
  headless capture always is — the root reads `Me`/`Já`, and the OG seed already has a
  branch by that name, so leaving it alone put the same word twice in one picture.
  `centerLabel` still overrides it. The ring is left exactly as it renders: the seed has
  no activities, so it is empty rather than showing invented progress.
- **No stranger's face anywhere.** The earlier worry was moot — every generated asset is
  captured signed-out, so the avatar tier never activates.

### Observation for the author, not a defect
With default branches, a signed-out user sees **"Me" twice** — once as the root, once as
the `Me` branch. Visible in `assets/screen-light-desktop-mindmap.png`. It follows from
the `Me`/`Já` wording and is real product behaviour, not a bug, so it is left alone.
Worth a look if it grates: `Me` on the root vs. a differently-named default branch.
