...
- Switch all SVG interaction from `mousedown/mousemove/mouseup` to **Pointer Events** (`pointerdown/pointermove/pointerup`) — handles both mouse and touch in one pass, `setPointerCapture` replaces the awkward global mousemove pattern
- Add pinch-to-zoom (two-pointer distance delta)
- Responsive toolbar: icon-only on narrow screens, possibly bottom-anchored

**Layer 2 — Action model (~3–4 hours):**
- Tap on node → **floating action bar** at bottom of screen (Done toggle, Counter +1/−1, Priority cycle). No long-press needed, no context menu on mobile.
